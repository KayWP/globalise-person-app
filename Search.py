...
def load_json_data(file_path):
    ...
    try:
        if not os.path.exists(file_path):
            st.error(f"File not found: {file_path}. Please ensure 'data.json' is in the same folder as this script.")
            return None
...
def main():
    ...
    # Automatically load data.json
    data_file = 'data.json'
    data = load_json_data(data_file)
    
    # Sidebar for navigation (File upload removed)
    with st.sidebar:
        st.header("Data Status")
        if data:
            st.success(f"Loaded: {data_file}")
        else:
            st.error(f"Waiting for {data_file}...")
        ...
        st.markdown("### About")
        st.markdown("""
        This app explores historical person records including:
        - Names and professional activities
        - Location relations and life events
        - Personal relations
        """)
    ...
    if data:
        ...
        st.warning(f"Please place a file named `{data_file}` in the application directory to begin.")
